...
1. Launch. An empty **Untitled** project opens with the welcome panel (Add Video…, Open the Sample
...
Tests: `FirstLaunchUITests.testWelcomeAndGettingStarted`, `testTourStepsThroughAndCanBeRepeated`,
...
3. Adding the same video file again is refused (status line "…is already part of…") and adding a
   different recording joins the same lane rather than stacking.
