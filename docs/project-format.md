...
| `stabilisation` | `{ "method": "off" / "motionData" / "gyroflow" / "picture", "smoothing": 0.5, "zoom": 1.15, "gyroflowFiles": [MediaReference] }` (#262, #263). `picture` steadies from the picture's own measured movement, kept outside the project (Application Support, by file path, size and date) and measured again only when the file changes. `gyroflow` shows Gyroflow's steadied copy of each file of the video (`gyroflowFiles`: the first file, then each clip) in place of the recording's picture while all of them exist; telemetry, clocks and chapters still come from the recording. Absent `gyroflowFiles` is none.: steadies the picture from the camera's recorded orientation (GoPro `CORI`). `smoothing` is the smoothed path's time constant in seconds (0.1–3); `zoom` enlarges the picture to hide the moved edges and caps each frame's move at `(zoom − 1) / (2 · zoom)` of the picture height. Absent in files saved before it, which is `off`. Not applied to footage recorded with HyperSmooth. |
