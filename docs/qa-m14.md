...
| # | Step | Expected |
|---|---|---|
| 1 | `swift run -c release onboard bench --project my.onboardproj --export --codec hevc --seconds 60` | Overlay ms/frame, export "× real time" ≥ 1 for 4K HEVC on Apple silicon, resident memory within a few MB of the start |
| 2 | `swift run -c release onboard bench --project my.onboardproj --size 3840x2160 --frames 150` | 4K overlay drawing under 25 ms/frame after the first (cache-building) frame |
...
- `onboard bench` (release): 11 overlays at 1080p 4.4–5.8 ms/frame (≈ 180 fps); 4K 17.8 ms/frame (56 fps). The first frame of each plan costs 90–135 ms building gauge faces and fonts.
