...
| 0.16.2 adding videos | `InputPlanningTests` (first recording opens a lane, duplicates skipped, a second recording appends to a single lane, Add Camera opens a lane, template objects bind to the inputs that arrive), `RecordingGapsTests` (no clock = no gap; real GoPro chapters butt together and recordings are minutes apart, gated on `OVERLAYGEN_SAMPLES_DIR`) | New from Template shows its gauges, they bind to the video and data added afterwards; two recordings play in sequence on one lane instead of stacking |
