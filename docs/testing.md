...
| M18 native RaceRender objects | `docs/qa-m18.md`; `IndicatorPanelTests` (ABS light off/on goldens, hold time, timing panel golden, text-data zones), `SpeedDeltaTests` | The ABS light lights amber when the channel crosses its level; the timing panel shows best/previous/current and both delta lanes; water/oil readouts turn amber/red at their thresholds |
